...
describe('Routes: GamificationProfiles', () => {
    ...
    before(async () => {
        try {
            ...
            console.log('Failure on Before from gamificationprofiles.patch test: ', err)
        }
    })
    after(async () => {
        try {
    ...
    describe('PATCH /gamificationprofiles/:gamificationprofiles.id', () => {

        afterEach(async () => {
            try {
                await gamificationDB.removeCollections()
            } catch (err) {
                console.log('Failure in gamificationprofiles.patch test: ', err.message)
            }
        })

        context('when the user posting a GamificationProfiles successfully', () => {

            it('gamificationprofiles.patch001: should return status code 200', () => {

                console.log(`/gamificationprofiles/${gamificationProfileId}`)
                return request(URI)
                    .patch(`/gamificationprofiles/${gamificationProfileId}`)
