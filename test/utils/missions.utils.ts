...
import { EducatorMissionsMock } from '../mocks/missions-service/educatorMissionsMock'
...
class missionsUtil {
    ...
    public saveEducatorMissions(accessToken: string, educatorMissions: EducatorMissionsMock): Promise<any> {

        return request(this.URI)
            .post(`/educator-missions`)
            .set('Authorization', 'Bearer '.concat(accessToken))
            .set('Content-Type', 'application/json')
            .send(educatorMissions)
            .then(res => Promise.resolve(res.body))
            .catch(err => Promise.reject(err))
    }
