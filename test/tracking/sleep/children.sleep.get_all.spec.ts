                ...
                return request(URI)
                    ...
                    .then(res => {
                        ...
                        SLEEP_RECORDS.forEach(function (sleep, index) {
                            ...
                            expect(res.body[index]).to.have.property('start_time', sleep.start_time!.toISOString().split('.')[0])
                            expect(res.body[index]).to.have.property('end_time', sleep.end_time!.toISOString().split('.')[0])
                            ...
                            sleep.pattern!.data_set.forEach(function (elem, index2) {
                                expect(res.body[index].pattern.data_set[index2].start_time).to.eql(elem.start_time.toISOString().split('.')[0])
                    ...
                    return request(URI)
                        ...
                        .then(res => {
                            ...
                            res.body.forEach(function (sleep_res, index) {
                                ...
                                expect(sleep_res).to.have.property('start_time', SLEEP_RECORDS[index].start_time!.toISOString().split('.')[0])
                                expect(sleep_res).to.have.property('end_time', SLEEP_RECORDS[index].end_time!.toISOString().split('.')[0])
                                ...
                                sleep_res.pattern!.data_set.forEach(function (elem, index2) {
                                    expect(elem.start_time).to.eql(SLEEP_RECORDS[index].pattern!.data_set[index2].start_time.toISOString().split('.')[0])
                    ...
                    return request(URI)
                        ...
                        .then(res => {
                            ...
                            res.body.forEach(function (sleep_res, index) { // when the amount of sleep is specified, iterate through the response
                                ...
                                expect(sleep_res).to.have.property('start_time', SLEEP_RECORDS_COPY[index].start_time!.toISOString().split('.')[0])
                                expect(sleep_res).to.have.property('end_time', SLEEP_RECORDS_COPY[index].end_time!.toISOString().split('.')[0])
                                ...
                                sleep_res.pattern!.data_set.forEach(function (elem, index2) {
                                    expect(elem.start_time).to.eql(SLEEP_RECORDS_COPY[index].pattern!.data_set[index2].start_time.toISOString().split('.')[0])
